...
docs.keyListenerStatus||docs.keydownInit();const checkBindings=(o,e)=>{const r=docs.keyArray;var m,n,t,s,i=r.some(o=>!keysThatAreUsed.includes(o.toString())),a=!isNaN(mode.number)&&mode.number<50?mode.number:1;for([m,n]of Object.entries(commandMap))for(const d of Object.entries(n))if(d[0]===o&&r.includes(m)&&("Escape"===m||!mode.isInMotion)){for(let o=0;o<a;o++)d[1]();mode.isInMotion||clearArray(r)}for([t,s]of Object.entries(motionsCommandMap))if(mode.isInMotion&&r.join("").replace(/,/g,"").includes(t)){for(let o=0;o<a;o++)s();clearArray(r),mode.isInMotion=!1}if(e)console.log("Overriding mode number",mode.numberS),mode.number=e;else{let e="";for(let o=0;o<r.length;o++)r[o].toString().match(/[0-9]/g)&&(console.log("I have a number"),e+=parseInt(r[o].toString()),console.log("Number is now",e));mode.number=parseInt(e),console.log(isNaN(mode.number)?1:mode.number,"mode.number")}"normal"===o&&i&&(fancyLogError("Not a valid key"),clearArray(r))};
